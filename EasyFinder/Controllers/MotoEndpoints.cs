...
public static class MotoEndpoints
{
    ...
    public static void Map(RouteGroupBuilder group)
    {
        ...
        // Predição com ML
        group.MapPost("/ml/motos/{chassi}/prob-manutencao",
                async (string chassi, ManutencaoInput? overrideInput, MottuDbContext db) =>
                {
                    // 1) Busca dados básicos da moto
                    var moto = await db.Motos.AsNoTracking().FirstOrDefaultAsync(m => m.Chassi == chassi);
                    if (moto is null)
                        return Results.NotFound(new { error = "Moto não encontrada pelo chassi informado." });
                    
                    var input = new ManutencaoInput
                    {
                        KmDesdeUltimaRevisao = overrideInput?.KmDesdeUltimaRevisao ?? 2000,
                        DiasDesdeUltimaRevisao = overrideInput?.DiasDesdeUltimaRevisao ?? 60,
                        IncidentesUltimos30d = overrideInput?.IncidentesUltimos30d ?? 0,
                        UsoDiarioMedioHoras = overrideInput?.UsoDiarioMedioHoras ?? 4,
                        IdadeEmMeses = overrideInput?.IdadeEmMeses ?? 24,
                        VelocidadeMedia = overrideInput?.VelocidadeMedia ?? 38,
                        TrocasOleoAtrasadas = overrideInput?.TrocasOleoAtrasadas ?? 0
                    };

                    // 3) Predição
                    var engine = ManutencaoMlService.GetOrCreate();
                    ...
                    return Results.Ok(new
                    {
                        chassi,
                        input,
                        vaiPrecisarManutencao = pred.VaiPrecisarManutencao,
                        probabilidade = pred.Probability,
                        score = pred.Score
                    });
                })
            .WithSummary("Probabilidade de manutenção da moto")
            .WithDescription(
                "Prevê a chance de a moto (chassi) requerer manutenção nos próximos períodos usando ML.NET.");
